...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
   ...
   * @return 페이징 생성 문자열
   */ 
  public String paging4(int recordCount, int nowPage, int recordPerPage, int devdiaryno, int devcateno){ 
    int pagePerBlock = 10; // 블럭당 페이지 수 
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동 
    if (nowGrp >= 2){ 
      str.append("<span class='span_box_1'><A href='./read.jsp?&nowPage="+_nowPage+"&devdiaryno=" + devdiaryno +"&devcateno="+devcateno+ "&recordPerPage="+recordPerPage+ "'>이전</A></span>"); 
    } 
    ...
    for(int i=startPage; i<=endPage; i++){ 
      ...
      if (nowPage == i){ 
        ...
        str.append("<span class='span_box_1'><A href='./read.jsp?&nowPage="+i+"&devdiaryno="+devdiaryno+"&devcateno="+devcateno+"&recordPerPage="+recordPerPage+"'>"+i+"</A></span>");   
      } 
    } 
     
    _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동 
    if (nowGrp < totalGrp){ 
      str.append("<span class='span_box_1'><A href='./read.jsp?&nowPage="+_nowPage+"&devdiaryno="+devdiaryno+"&devcateno="+devcateno+"&recordPerPage="+recordPerPage+"'>다음</A></span>"); 
    } 
    str.append("</DIV>"); 
