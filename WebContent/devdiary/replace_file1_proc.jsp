...
int count = 0;
int devcateno = Integer.parseInt(upload.toKor(upload.getParameter("devcateno")));
int devdiaryno = Integer.parseInt(upload.toKor(upload.getParameter("devdiaryno")));
...
if (devdiaryProc.passwordCheck(devdiaryno, passwd) == 1) { // 패스워드 일치
  ...
  if (size1 > 0) {
    if (size1 < 10485760) { // 1 ~ 10485760 byte
      ...
      // 기존 파일 삭제 
      // -----------------------------------------------------
      DevdiaryVO  olddevdiaryVO = devdiaryProc.read(devdiaryno);   // 등록된 파일정보 읽어오기
      Tool.deleteFile(upDir, olddevdiaryVO.getFstor1());  // 저장된 파일 삭제
      Tool.deleteFile(upDir, olddevdiaryVO.getThumb()); // thumb 삭제
      // -----------------------------------------------------
...
%>
  ...
  <DIV class='bottom_menu'>
    ...
    %>   
    <button type='button' onclick="location.href='./list.jsp?devcateno=<%=devcateno %>'">뒤로가기</button>
  </DIV>
